...
const signup = async (req, res) => {
  try {
    ...
    console.log(result);
    // sendMail(req.body.email, "Seller account created", req.body.name, "Welcome to KisaanSetu");
    ...
    if (error.code === 11000) {
      if (error.keyPattern.email || error.keyPattern.phoneNo) {
        return res
          .status(400)
          .send({
            message: "Seller with this email or phone number already exists",
          });
...
const deleteSellerAccount = async (req, res) => {
  try {
    ...
    await Promise.all([Product.deleteMany({ sellerId: req.params.sellerId })]);

    let data = await Seller.deleteOne(
